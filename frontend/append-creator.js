...
async function renderCreatorStudio() {
  ...
  html += \`
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      <div class="glass p-4 rounded-2xl text-center">
        <div class="text-2xl font-bold text-indigo-500">\${scripts.length}</div>
        <div class="text-xs text-gray-500 mt-1">脚本总数</div>
      </div>
      <div class="glass p-4 rounded-2xl text-center">
        <div class="text-2xl font-bold text-green-500">\${scripts.filter(function(s){return s.status==='done'}).length}</div>
        <div class="text-xs text-gray-500 mt-1">已完成</div>
      </div>
      <div class="glass p-4 rounded-2xl text-center">
        <div class="text-2xl font-bold text-amber-500">\${scripts.filter(function(s){return s.status==='draft'}).length}</div>
        <div class="text-xs text-gray-500 mt-1">草稿</div>
      </div>
      <div class="glass p-4 rounded-2xl text-center">
        <div class="text-2xl font-bold text-purple-500">\${scripts.filter(function(s){return s.status==='published'}).length}</div>
  ...
  if (scripts.length === 0) {
    ...
    scripts.forEach(function(s) {
      ...
      html += \`
        <div class="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl flex items-center justify-between">
...
// 追加到 app.js
fs.appendFileSync(APP_JS, creatorCode, 'utf8');
console.log('✅ 创作者工作室功能已追加到 app.js');
